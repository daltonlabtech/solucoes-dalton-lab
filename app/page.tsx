    ...
    <div className="bg-[#F8FAFC] min-h-screen">
      ...
      <section className="relative flex flex-col items-center justify-center text-center px-6 py-28 overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(circle at 50% 40%, rgba(51,173,229,0.07) 0%, transparent 65%)' }}
          aria-hidden="true"
        />
      ...
      <section id="solucoes" className="py-16 px-6" aria-labelledby="produtos-title">
        <div className="max-w-6xl mx-auto flex flex-col gap-6">
          ...
          <article className="relative bg-white rounded-2xl p-8 flex flex-col md:flex-row md:items-center gap-6 shadow-md border border-slate-200 overflow-hidden hover:shadow-lg transition-shadow duration-200">
            {/* Accent top line */}
            ...
            <div className="flex-1 flex flex-col gap-3 mt-1">
              <div className="flex items-center gap-3">
                <h3 className="text-2xl font-black text-slate-900">{featuredProduct.name}</h3>
                <span className="text-xs font-bold uppercase tracking-widest text-green-600 bg-green-50 border border-green-200 px-3 py-1 rounded-full">
                  Disponível agora
                </span>
              </div>
              <p className="text-slate-600 leading-relaxed text-lg">{featuredProduct.tagline}</p>
              <p className="text-slate-400 text-sm">{featuredProduct.metric} · {featuredProduct.price}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {comingSoonProducts.map(p => (
              <article
                key={p.href}
                className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col gap-3 hover:border-dalton-cyan/40 hover:shadow-sm transition-all duration-200"
              >
                ...
                <p className="text-slate-600 text-sm leading-relaxed">{p.tagline}</p>

                <p className="text-xs text-slate-400 italic border-l-2 border-dalton-cyan/40 pl-3 leading-relaxed">
                  {p.painStat}
                </p>
      ...
      <section className="py-12 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-slate-500 text-lg leading-relaxed text-center">
            Mais de <span className="text-slate-900 font-bold">3 milhões de visualizações</span> já rastreadas pelo Radar. Cada ferramenta que lançamos resolve um problema específico — com dado, não com promessa.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-6 bg-slate-50">
        <div className="max-w-3xl mx-auto text-center flex flex-col items-center gap-6">
          <p className="text-slate-500 text-lg leading-relaxed">
            ...
            <span className="text-slate-900 font-medium">Se não funcionar no seu negócio, você cancela. Sem multa, sem fidelidade.</span>
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            {['Sem contrato anual', 'Setup em horas', 'Suporte direto com o time'].map(item => (
              <div key={item} className="flex items-center gap-2 text-sm text-slate-600 bg-white border border-slate-200 px-4 py-2 rounded-full shadow-sm">
